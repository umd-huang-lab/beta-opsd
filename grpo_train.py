...
def extract_ground_truth_answer(example):
    """Return an answer string from common math dataset schemas."""
    ...
    if isinstance(refs, list) and refs:
        ...
        if isinstance(first_ref, dict):
            for key in ("answer", "final_answer", "solution"):
                if first_ref.get(key):
                    if key == "solution":
                        boxed = extract_boxed_answer(str(first_ref[key]))
                        return boxed or str(first_ref[key])
                    return str(first_ref[key])
        elif isinstance(first_ref, str):
            boxed = extract_boxed_answer(first_ref)
            return boxed or first_ref
...
def make_format_prompt(tokenizer):
    ...
    def format_prompt(example):
        ...
        answer = extract_ground_truth_answer(example)
...
if __name__ == "__main__":
    ...
    print(f"Max Prompt Length: {getattr(training_args, 'max_prompt_length', None)}")
    ...
    train_dataset = train_dataset.filter(lambda example: example["prompt"] is not None and example["Answer"] is not None)
