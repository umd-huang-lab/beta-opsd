...
class BetaOPSDTrainer(SFTTrainer):
    _tag_names = ["trl", "opsd", "beta_opsd"]
    _name = "BetaOPSD"
    ...
    def __init__(
        ...
        use_thinking_machines_loss: bool = True,
        tinker_use_reward_to_go: bool = False,
        tinker_reward_to_go_discount: float = 1.0,
        tinker_advantage_clip: float | None = None,
        use_mixed_teacher_target: bool = True,
        ...
        mixed_teacher_target_reference_model: str = "frozen_reference",
        use_mixed_sampling: bool = False,
        mixed_sampling_teacher_alpha: float = 0.0,
        mixed_sampling_teacher_alpha_linear_decay: bool = False,
        mixed_sampling_teacher_alpha_exponential_decay: bool = False,
        mixed_sampling_teacher_alpha_final: float = 0.0,
        use_per_token_importance_sampling: bool = False,
        ...
        self.tinker_use_reward_to_go = tinker_use_reward_to_go
        self.tinker_reward_to_go_discount = tinker_reward_to_go_discount
        self.tinker_advantage_clip = tinker_advantage_clip
        if use_mixed_sampling:
            fixed_teacher = True
            use_mixed_teacher_target = False

        self.use_mixed_teacher_target = use_mixed_teacher_target
        self.mixed_teacher_target_teacher_weight = mixed_teacher_target_teacher_weight
        self.mixed_teacher_target_teacher_weight_linear_decay = mixed_teacher_target_teacher_weight_linear_decay
        self.mixed_teacher_target_teacher_weight_final = mixed_teacher_target_teacher_weight_final
        self.mixed_teacher_target_reference_model = mixed_teacher_target_reference_model
        self.use_mixed_sampling = use_mixed_sampling
        self.mixed_sampling_teacher_alpha = mixed_sampling_teacher_alpha
        self.mixed_sampling_teacher_alpha_linear_decay = mixed_sampling_teacher_alpha_linear_decay
        self.mixed_sampling_teacher_alpha_exponential_decay = mixed_sampling_teacher_alpha_exponential_decay
        self.mixed_sampling_teacher_alpha_final = mixed_sampling_teacher_alpha_final
        self.use_per_token_importance_sampling = use_per_token_importance_sampling
        self.importance_sampling_clip_min = importance_sampling_clip_min
        self.importance_sampling_clip_max = importance_sampling_clip_max
        self.use_teacher_proposal_vllm = False
        ...
        if self.tinker_advantage_clip is not None and self.tinker_advantage_clip <= 0.0:
            raise ValueError("tinker_advantage_clip must be positive when set.")
        if self.use_mixed_teacher_target and not self.use_thinking_machines_loss:
            raise ValueError("use_mixed_teacher_target=True requires use_tinker_loss=True.")
        if self.mixed_teacher_target_reference_model not in {"frozen_reference", "current_student"}:
            raise ValueError("mixed_teacher_target_reference_model must be 'frozen_reference' or 'current_student'.")
        if (
            ...
            raise ValueError(
                "use_mixed_teacher_target=True requires a PEFT config (use_peft=True). "
                "The reference student is implemented by disabling LoRA adapters."
            )
        ...
        if not 0.0 <= self.mixed_sampling_teacher_alpha <= 1.0:
            raise ValueError("mixed_sampling_teacher_alpha must be in [0, 1].")
        if not 0.0 <= self.mixed_sampling_teacher_alpha_final <= 1.0:
            raise ValueError("mixed_sampling_teacher_alpha_final must be in [0, 1].")
        if self.mixed_sampling_teacher_alpha_linear_decay and self.mixed_sampling_teacher_alpha_exponential_decay:
            raise ValueError("Only one alpha schedule may be enabled at a time.")
        if self.use_mixed_sampling and self.mixed_sampling_teacher_alpha <= 0.0:
            raise ValueError("use_mixed_sampling=True requires mixed_sampling_teacher_alpha > 0.")
        if self.mixed_sampling_teacher_alpha_exponential_decay and self.mixed_sampling_teacher_alpha_final <= 0.0:
            raise ValueError("Exponential alpha decay requires mixed_sampling_teacher_alpha_final > 0.")
        if (
            ...
            raise ValueError("importance_sampling_clip_min must be <= importance_sampling_clip_max.")
        if (
            self.use_per_token_importance_sampling
            and getattr(args, "use_vllm", False)
            and getattr(args, "vllm_mode", None) == "server"
        ):
            raise ValueError(
                "Per-token importance sampling requires proposal logprobs. "
                "It is supported for HF generation and vLLM colocate mode, not vLLM server mode."
            )
        if (
            self.use_mixed_sampling
            and getattr(args, "use_vllm", False)
            and not self.mixed_sampling_teacher_alpha_linear_decay
            and self.mixed_sampling_teacher_alpha >= 1.0
        ):
            if not self.fixed_teacher:
                raise ValueError(
                    "Teacher-only proposal sampling with vLLM requires fixed_teacher=True."
                )
            self.use_teacher_proposal_vllm = True
        ...
        if self.use_mixed_sampling:
            print(f"\n{'='*80}")
            print("MIXED SAMPLING ENABLED")
            print(f"loss: {'tinker' if self.use_thinking_machines_loss else 'jsd'}")
            print(f"teacher_alpha_start: {self.mixed_sampling_teacher_alpha}")
            print("mixture_space: pre_softmax")
            if self.mixed_sampling_teacher_alpha_linear_decay:
                print(
                    f"teacher_alpha_schedule: linear_decay_to_{self.mixed_sampling_teacher_alpha_final}"
                )
            elif self.mixed_sampling_teacher_alpha_exponential_decay:
                print(
                    f"teacher_alpha_schedule: exponential_decay_to_{self.mixed_sampling_teacher_alpha_final}"
                )
            else:
                print("teacher_alpha_schedule: constant")
            print(f"teacher_alpha_current: {self._current_mixed_sampling_teacher_alpha()}")
            print(f"use_vllm: {getattr(args, 'use_vllm', False)}")
            print(f"vllm_mode: {getattr(args, 'vllm_mode', 'disabled')}")
            print(
                "generation: "
                f"temperature={self.temperature}, top_p={self.top_p}, top_k={args.top_k}"
            )
            print(f"jsd_token_clip: {self.jsd_token_clip if self.jsd_token_clip is not None else 'off'}")
            print(
                "tinker_advantage_clip: "
                f"{self.tinker_advantage_clip if self.tinker_advantage_clip is not None else 'off'}"
            )
            print(f"top_k_loss: {self.top_k_loss if self.top_k_loss is not None else 'full_vocab'}")
            print(f"teacher_proposal_vllm_fast_path: {self.use_teacher_proposal_vllm}")
            print(f"per_token_importance_sampling: {self.use_per_token_importance_sampling}")
            if self.use_per_token_importance_sampling:
                print(
                    "importance_sampling_clip: "
                    f"min={self.importance_sampling_clip_min if self.importance_sampling_clip_min is not None else 'off'}, "
                    f"max={self.importance_sampling_clip_max if self.importance_sampling_clip_max is not None else 'off'}"
                )
            print(f"{'='*80}\n")
        ...
        self.use_transformers_paged = getattr(args, "use_transformers_paged", False) or False
        ...
        # Previously raised NotImplementedError here — now supported via vLLM shadow requests.
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                self.vllm_engine = LLM(
                    ...
                    * self.args.gradient_accumulation_steps,
                    ...
                    # Importance sampling needs proposal logprobs from the same
                    # processed distribution used by the sampler after
                    # temperature/top-k/top-p/min-p transforms.
    ...
    def _current_mixed_sampling_teacher_alpha(self) -> float:
        if not self.use_mixed_sampling:
            return 0.0
        if not self.mixed_sampling_teacher_alpha_linear_decay and not self.mixed_sampling_teacher_alpha_exponential_decay:
            return self.mixed_sampling_teacher_alpha

        total_steps = getattr(self.state, "max_steps", 0) or getattr(self.args, "max_steps", 0) or 0
        if total_steps <= 1:
            return self.mixed_sampling_teacher_alpha_final

        progress = min(max(self.state.global_step / (total_steps - 1), 0.0), 1.0)
        if self.mixed_sampling_teacher_alpha_exponential_decay:
            start = self.mixed_sampling_teacher_alpha
            end = self.mixed_sampling_teacher_alpha_final
            ratio = end / start
            return start * (ratio**progress)
        return (
            self.mixed_sampling_teacher_alpha
            + (self.mixed_sampling_teacher_alpha_final - self.mixed_sampling_teacher_alpha) * progress
        )
    ...
    def _current_mixed_teacher_target_teacher_weight(self) -> float:
        ...
        return (
            ...
            + (self.mixed_teacher_target_teacher_weight_final - self.mixed_teacher_target_teacher_weight) * progress
        )

    def _mixed_sampling_alpha_progress(self) -> float:
        if (
            not self.use_mixed_sampling
            or not (
                self.mixed_sampling_teacher_alpha_linear_decay
                or self.mixed_sampling_teacher_alpha_exponential_decay
            )
        ):
            return 0.0
        total_steps = getattr(self.state, "max_steps", 0) or getattr(self.args, "max_steps", 0) or 0
        if total_steps <= 1:
            return 1.0
        return min(max(self.state.global_step / (total_steps - 1), 0.0), 1.0)

    @staticmethod
    def _extract_vllm_sample_logprobs(outputs) -> list[list[float]] | None:
        """Extract sampled-token logprobs from vLLM CompletionOutput objects."""
        proposal_logprobs = []
        for output in outputs:
            ...
            if output_logprobs is None:
                return None
            token_logprobs = []
            for token_id, step_logprobs in zip(token_ids, output_logprobs):
                ...
                if isinstance(step_logprobs, dict):
                    ...
                    if token_info is None:
                        token_info = step_logprobs.get(int(token_id))
                    if token_info is not None:
                        token_logprob = getattr(token_info, "logprob", token_info)
                else:
                    # FlatLogprobs stores sampled-token logprob first for each
                    # position when logprobs=0.
                    try:
                        token_logprob = step_logprobs[0]
                        token_logprob = getattr(token_logprob, "logprob", token_logprob)
                    except (TypeError, IndexError):
                        return None
                if token_logprob is None:
                    return None
                token_logprobs.append(float(token_logprob))
            proposal_logprobs.append(token_logprobs)
    ...
    @staticmethod
    def _apply_sampling_filters(log_probs: torch.Tensor, top_k: int | None, top_p: float | None) -> torch.Tensor:
        filtered = log_probs
        ...
        if top_p is not None and 0.0 < top_p < 1.0:
            sorted_log_probs, sorted_idx = torch.sort(filtered, descending=True, dim=-1)
            sorted_probs = torch.softmax(sorted_log_probs, dim=-1)
        ...
        return filtered - torch.logsumexp(filtered, dim=-1, keepdim=True)
    ...
    def _mix_log_probs(
        ...
        if teacher_alpha <= 0.0:
            return F.log_softmax(student_logits, dim=-1)
        if teacher_alpha >= 1.0:
            return F.log_softmax(teacher_logits, dim=-1)
        alpha = torch.tensor(teacher_alpha, dtype=student_logits.dtype, device=student_logits.device)
        mixed_logits = (1.0 - alpha) * student_logits + alpha * teacher_logits
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_thinking_machines_loss:
            # For reverse-KL-style losses, we only need log-probs of sampled tokens
        ...
        teacher_proposal_log_probs_sampled = None
        with torch.no_grad(), adapter_context:
            ...
            if self.use_thinking_machines_loss:
                ...
                if self.use_teacher_proposal_vllm and self.use_per_token_importance_sampling:
                    top_k = (
                        self.generation_config.top_k
                        if self.generation_config.top_k and self.generation_config.top_k > 0
                        else None
                    )
                    top_p = self.generation_config.top_p if self.generation_config.top_p is not None else 1.0
                    teacher_proposal_log_probs = self._apply_sampling_filters(
                        teacher_log_probs, top_k=top_k, top_p=top_p
                    )
                    teacher_proposal_log_probs_sampled = torch.gather(
                        teacher_proposal_log_probs, dim=-1, index=sampled_token_ids.unsqueeze(-1)
                    ).squeeze(-1)
        ...
        target_teacher_log_probs_sampled = teacher_log_probs_sampled if self.use_thinking_machines_loss else None
        reference_log_probs_sampled = None
        if self.use_thinking_machines_loss and self.use_mixed_teacher_target:
            if self.mixed_teacher_target_reference_model == "current_student":
                ...
                with torch.no_grad(), self.accelerator.unwrap_model(model).disable_adapter():
                    ...
                    reference_logits = outputs_reference.logits[:, student_prompt_len - 1 : -1, :]
                    reference_log_probs = F.log_softmax(reference_logits / self.temperature, dim=-1)
                    reference_log_probs_sampled = torch.gather(
                        reference_log_probs, dim=-1, index=sampled_token_ids.unsqueeze(-1)
                    ).squeeze(-1)
                    del outputs_reference, reference_logits, reference_log_probs
                    empty_cache()

            target_teacher_log_probs_sampled = self._mix_sampled_log_probs(
        ...
        if self.use_per_token_importance_sampling:
            proposal_log_probs_sampled = inputs.get("proposal_log_probs_sampled", None)
            if proposal_log_probs_sampled is None:
                proposal_log_probs_sampled = teacher_proposal_log_probs_sampled
            if proposal_log_probs_sampled is not None:
                importance_weights = torch.exp(
                    student_log_probs_sampled.detach() - proposal_log_probs_sampled
                )
                if self.importance_sampling_clip_min is not None:
                    importance_weights = importance_weights.clamp_min(self.importance_sampling_clip_min)
                if self.importance_sampling_clip_max is not None:
                    importance_weights = importance_weights.clamp_max(self.importance_sampling_clip_max)

                mode = "train" if model.training else "eval"
                if shifted_labels is not None:
                    imp_mask = shifted_labels != -100
                    self._metrics[mode]["importance_weight"].append(
                        importance_weights[imp_mask].mean().detach().item()
                    )
                else:
                    self._metrics[mode]["importance_weight"].append(importance_weights.mean().detach().item())
        ...
        if self.use_thinking_machines_loss:
            ...
            advantage = (target_teacher_log_probs_sampled - student_log_probs_sampled).detach()
            ...
            if self.tinker_advantage_clip is not None:
                advantage = advantage.clamp(
                    min=-self.tinker_advantage_clip,
                    max=self.tinker_advantage_clip,
                )
            if importance_weights is not None:
                advantage = advantage * importance_weights.detach()
            loss = -(advantage * student_log_probs_sampled_masked).mean()

            del (
                student_log_probs_sampled,
                teacher_log_probs_sampled,
                target_teacher_log_probs_sampled,
                advantage,
                student_log_probs_sampled_masked,
                importance_weights,
            )
            if reference_log_probs_sampled is not None:
                del reference_log_probs_sampled
        else:
    ...
    def generate_mixed_sampling_outputs(self, model, inputs, generation_config, pad_token_id=None):
        """Generate completions from a student-teacher proposal mixture and return sampled proposal log-probs."""
        import time
        ...
        student_input_ids = inputs["student_prompts"]
        student_attention_mask = inputs.get("student_prompt_attention_mask", None)
        if student_attention_mask is None:
            student_attention_mask = torch.ones_like(student_input_ids, device=device)
        teacher_input_ids = inputs["teacher_prompts"]
        teacher_attention_mask = inputs.get("teacher_prompt_attention_mask", None)
        if teacher_attention_mask is None:
            teacher_attention_mask = torch.ones_like(teacher_input_ids, device=device)

        max_new_tokens = generation_config.max_new_tokens
        temperature = generation_config.temperature
        top_k = generation_config.top_k if generation_config.top_k and generation_config.top_k > 0 else None
        ...
        try:
            with torch.no_grad():
                ...
                student_past = student_outputs.past_key_values
                student_next_logits = student_outputs.logits[:, -1, :]
                del student_outputs

                if self.use_ema_teacher:
                    teacher_context_factory = lambda: self._ema_teacher_context(model)
                elif self.fixed_teacher and is_peft_model(model):
                    teacher_context_factory = lambda: self.accelerator.unwrap_model(model).disable_adapter()
                else:
                    teacher_context_factory = nullcontext

                with teacher_context_factory():
                    ...
                    teacher_next_logits = teacher_outputs.logits[:, -1, :]
                    del teacher_outputs

                sampled_tokens = []
                sampled_proposal_log_probs = []
                current_student_attention = student_attention_mask
                current_teacher_attention = teacher_attention_mask

                for _ in range(max_new_tokens):
                    proposal_log_probs = self._mix_log_probs(
                        student_logits=student_next_logits / temperature,
                        teacher_logits=teacher_next_logits / temperature,
                        teacher_alpha=self._current_mixed_sampling_teacher_alpha(),
                    )
                    proposal_log_probs = self._apply_sampling_filters(
                        proposal_log_probs, top_k=top_k, top_p=top_p
                    )

                    next_token = torch.multinomial(proposal_log_probs.exp(), num_samples=1).squeeze(-1)
                    next_token_log_prob = torch.gather(
                        proposal_log_probs, dim=-1, index=next_token.unsqueeze(-1)
                    ).squeeze(-1)

                    sampled_tokens.append(next_token)
                    sampled_proposal_log_probs.append(next_token_log_prob)

                    next_token_input = next_token.unsqueeze(-1)
                    current_student_attention = torch.cat(
                        [
                            current_student_attention,
                            torch.ones(
                                (next_token.size(0), 1),
                                dtype=current_student_attention.dtype,
                                device=device,
                            ),
                        ],
                        dim=1,
                    ...
                    student_outputs = model(
                        ...
                        attention_mask=current_student_attention,
                    ...
                    current_teacher_attention = torch.cat(
                        [
                            current_teacher_attention,
                            torch.ones(
                                (next_token.size(0), 1),
                                dtype=current_teacher_attention.dtype,
                                device=device,
                            ),
                        ],
                        dim=1,
                    ...
                    with teacher_context_factory():
                        teacher_outputs = model(
                            input_ids=next_token_input,
                            attention_mask=current_teacher_attention,
                ...
                proposal_log_probs_sampled = torch.stack(sampled_proposal_log_probs, dim=1)
                generated_tokens = torch.cat([student_input_ids, completion_ids], dim=1)
                new_attention_mask = torch.cat(
                    [
                        student_attention_mask,
                        torch.ones_like(completion_ids, dtype=student_attention_mask.dtype),
                    ],
                    dim=1,
                )
                new_labels = generated_tokens.clone()
        ...
        elapsed_time = time.time() - start_time
        num_prompts = generated_tokens.shape[0]
        total_completion_tokens = completion_ids.numel()
        avg_completion_length = completion_ids.shape[1]
        tokens_per_sec = total_completion_tokens / elapsed_time if elapsed_time > 0 else 0
        print(
            f"mixed proposal generation done - elapsed time: {elapsed_time:.2f}s, prompts: {num_prompts}, "
            f"total tokens: {total_completion_tokens}, avg length: {avg_completion_length}, speed: {tokens_per_sec:.1f} tok/s"
        )

        return generated_tokens, new_attention_mask, new_labels, proposal_log_probs_sampled

    @profiling_decorator
    def _generate_on_policy_outputs_vllm(self, inputs, generation_config, pad_token_id=None, mixture_alpha=0.0):
        ...
        need_proposal_logprobs = (
            self.use_per_token_importance_sampling
            and mixture_alpha > 0.0
        )
        proposal_log_probs_sampled = None
        ...
        if self.vllm_mode == "server":
            ...
            if need_proposal_logprobs:
                # logprobs=0 asks vLLM to return only the sampled token's
                # logprob at each step. The engine was configured with
                # processed_logprobs, so these are proposal logprobs after
                # temperature/top-k/top-p/min-p filtering.
                base_sampling_kwargs["logprobs"] = 0
            ...
            if hasattr(self, "vllm_tp_group") and self.vllm_tensor_parallel_size > 1:
                ...
                gathered_prompts_list = [None for _ in range(self.vllm_tensor_parallel_size)]
                torch.distributed.all_gather_object(
                    gathered_prompts_list, prompts_text_for_vllm, group=self.vllm_tp_group
                )
                all_prompts_text = [p for sublist in gathered_prompts_list for p in sublist]

                if mixture_alpha > 0.0 and "teacher_prompts" in inputs:
                    gathered_teacher = [None for _ in range(self.vllm_tensor_parallel_size)]
                    torch.distributed.all_gather_object(
                        gathered_teacher, inputs["teacher_prompts"].tolist(), group=self.vllm_tp_group
                    )
                    all_teacher_token_ids = [ids for sublist in gathered_teacher for ids in sublist]
                else:
                    all_teacher_token_ids = None
            else:
                all_prompts_text = prompts_text_for_vllm
                if mixture_alpha > 0.0 and "teacher_prompts" in inputs:
                    all_teacher_token_ids = inputs["teacher_prompts"].tolist()
                else:
                    all_teacher_token_ids = None

            if all_teacher_token_ids is not None:
                # Per-prompt SamplingParams with teacher token IDs for mixture sampling.
                    ...
                    SamplingParams(
                        **base_sampling_kwargs,
                        ...
                        teacher_prompt_token_ids=[
                            t for t in teacher_ids if t != pad_id
                        ],
                    )
                    for teacher_ids in all_teacher_token_ids
                ]
                all_outputs = self.vllm_engine.generate(
                    all_prompts_text, sampling_params=sampling_params_list, use_tqdm=False
                )
            else:
            ...
            completion_outputs = [output for outputs in all_outputs for output in outputs.outputs]
            completion_ids = [output.token_ids for output in completion_outputs]
            proposal_log_probs_sampled = (
                self._extract_vllm_sample_logprobs(completion_outputs)
                if need_proposal_logprobs
                else None
            )
            ...
            if hasattr(self, "vllm_tp_group") and self.vllm_tensor_parallel_size > 1:
                ...
                if proposal_log_probs_sampled is not None:
                    proposal_log_probs_sampled = proposal_log_probs_sampled[tp_slice]
        ...
        if need_proposal_logprobs and proposal_log_probs_sampled is None:
            raise RuntimeError(
                "Per-token importance sampling was enabled, but vLLM did not return "
                "sampled-token proposal logprobs. Ensure vLLM colocate mode is used "
                "with logprobs support enabled."
            )
        ...
        padded_proposal_log_probs_list = []
        for idx, completion_tensor in enumerate(completion_ids_tensors):
            if len(completion_tensor) > max_completion_length:
                ...
                if proposal_log_probs_sampled is not None:
                    padded_proposal_log_probs_list.append(
                        torch.tensor(
                            proposal_log_probs_sampled[idx][:max_completion_length],
                            device=device,
                            dtype=torch.float32,
                        )
                    )
                ...
                if proposal_log_probs_sampled is not None:
                    logprob_tensor = torch.tensor(
                        proposal_log_probs_sampled[idx],
                        device=device,
                        dtype=torch.float32,
                    )
                    padded_proposal_log_probs_list.append(
        ...
        padded_proposal_log_probs = (
            torch.stack(padded_proposal_log_probs_list)
            if padded_proposal_log_probs_list
            else None
        )
    ...
    @profiling_decorator
    def _generate_teacher_proposal_outputs_vllm(self, inputs, generation_config, pad_token_id=None):
        """Generate completions from teacher prompts using vLLM and attach them to student prompts."""
        import time

        device = self.accelerator.device

        teacher_prompts_text_for_vllm = self.processing_class.batch_decode(
            inputs["teacher_prompts"],
            skip_special_tokens=False,
        )
        if self.processing_class.pad_token:
            teacher_prompts_text_for_vllm = [
                p.replace(self.processing_class.pad_token, "") for p in teacher_prompts_text_for_vllm
            ]
        ...
        if self.vllm_mode == "server":
            all_prompts_text = gather_object(teacher_prompts_text_for_vllm)
            if self.accelerator.is_main_process:
                completion_ids = self.vllm_client.generate(
                    prompts=all_prompts_text,
                    n=1,
            ...
            completion_ids = broadcast_object_list(completion_ids, from_process=0)
            process_slice = slice(
                self.accelerator.process_index * len(teacher_prompts_text_for_vllm),
                (self.accelerator.process_index + 1) * len(teacher_prompts_text_for_vllm),
            )
            completion_ids = completion_ids[process_slice]
        elif self.vllm_mode == "colocate":
            if self.vllm_guided_decoding_regex:
            ...
            if hasattr(self, "vllm_tp_group") and self.vllm_tensor_parallel_size > 1:
                orig_size = len(teacher_prompts_text_for_vllm)
                gathered_prompts = [None for _ in range(self.vllm_tensor_parallel_size)]
                torch.distributed.all_gather_object(
                    gathered_prompts, teacher_prompts_text_for_vllm, group=self.vllm_tp_group
                )
                all_prompts_text = [p for sublist in gathered_prompts for p in sublist]
            else:
                all_prompts_text = teacher_prompts_text_for_vllm
        ...
        tokens_per_sec = total_completion_tokens / elapsed_time if elapsed_time > 0 else 0
        print(
            f"teacher-proposal vLLM generation done - elapsed time: {elapsed_time:.2f}s, prompts: {num_prompts}, "
            f"total tokens: {total_completion_tokens}, avg length: {avg_completion_length:.1f}, speed: {tokens_per_sec:.1f} tok/s"
        )
        ...
        padded_completion_ids = torch.stack(padded_completion_ids_list)
        student_prompt_ids = inputs["student_prompts"]
        student_prompt_attention_mask = inputs.get("student_prompt_attention_mask", None)
        if student_prompt_attention_mask is None:
            student_prompt_attention_mask = torch.ones_like(student_prompt_ids, device=device)

        if student_prompt_ids.ndim == 1:
            student_prompt_ids = student_prompt_ids.unsqueeze(0)
        if padded_completion_ids.ndim == 1:
            padded_completion_ids = padded_completion_ids.unsqueeze(0)

        new_input_ids = torch.cat([student_prompt_ids, padded_completion_ids], dim=1)
        completion_attention_mask = torch.ones_like(padded_completion_ids, device=device)
        if pad_token_id is not None:
            completion_attention_mask[padded_completion_ids == pad_token_id] = 0
        new_attention_mask = torch.cat([student_prompt_attention_mask, completion_attention_mask], dim=1)
        new_labels = new_input_ids.clone()
    ...
    def _generate_teacher_reasoning_vllm(
        ...
        if self.vllm_mode == "server":
            ...
            if self.accelerator.is_main_process:
                ...
                completion_ids = result["completion_ids"] if isinstance(result, dict) else result
            else:
                completion_ids = [None] * len(all_prompts_text)
            completion_ids = broadcast_object_list(completion_ids, from_process=0)
            process_slice = slice(
            ...
            completion_ids = list(completion_ids)[process_slice]
    ...
    def training_step(
        ...
        # === GENERATION PHASE ===
        inputs.pop("proposal_log_probs_sampled", None)
        current_teacher_alpha = self._current_mixed_sampling_teacher_alpha()
        if self.use_vllm:
            self._wake_vllm_if_needed()
            if self.use_teacher_proposal_vllm and current_teacher_alpha >= 1.0:
                result = self._generate_teacher_proposal_outputs_vllm(
                ...
                vllm_mixture_alpha = current_teacher_alpha if self.use_mixed_sampling else 0.0
                result = self._generate_on_policy_outputs_vllm(
                    inputs, self.generation_config, self.processing_class.pad_token_id,
                    mixture_alpha=vllm_mixture_alpha,
                )
                (
                    generated_ids,
                ...
                if proposal_log_probs_sampled is not None:
                    inputs["proposal_log_probs_sampled"] = proposal_log_probs_sampled
        else:
            with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                if self.use_mixed_sampling:
                    result = self.generate_mixed_sampling_outputs(
                        unwrapped_model, inputs, self.generation_config, self.processing_class.pad_token_id
                    )
                    generated_ids, generated_attention_mask, _, proposal_log_probs_sampled = result
                    inputs["proposal_log_probs_sampled"] = proposal_log_probs_sampled
                else:
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        if mode == "train":
            ...
            if self.use_mixed_sampling:
                logs["teacher_alpha"] = round(self._current_mixed_sampling_teacher_alpha(), 6)
                if (
                    self.mixed_sampling_teacher_alpha_linear_decay
                    or self.mixed_sampling_teacher_alpha_exponential_decay
                ):
                    logs["teacher_alpha_progress"] = round(self._mixed_sampling_alpha_progress(), 6)
                    logs["teacher_alpha_target"] = round(self.mixed_sampling_teacher_alpha_final, 6)
            if self.use_mixed_teacher_target:
                logs["target_teacher_weight"] = round(self._current_mixed_teacher_target_teacher_weight(), 6)
                if self.mixed_teacher_target_teacher_weight_linear_decay:
                    logs["target_teacher_weight_target"] = round(self.mixed_teacher_target_teacher_weight_final, 6)
            if self.top_k_loss is not None:
                logs["top_k_loss"] = float(self.top_k_loss)
            if self.jsd_token_clip is not None:
                logs["jsd_token_clip"] = float(self.jsd_token_clip)
