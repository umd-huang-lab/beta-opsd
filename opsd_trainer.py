...
class OPSDTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        if self.use_vllm:
            ...
            self.vllm_guided_decoding_regex = args.vllm_structured_outputs_regex
        ...
        if self.vllm_mode == "server":
            ...
            if self.accelerator.is_main_process:
                result = self.vllm_client.generate(
                    ...
                    structured_outputs_regex=self.vllm_guided_decoding_regex,
                    generation_kwargs={"presence_penalty": presence_penalty} if presence_penalty else None,
                )
                # TRL 1.x returns a dict; older versions returned a list directly
            ...
            structured_outputs = (
                StructuredOutputsParams(regex=self.vllm_guided_decoding_regex)
                if self.vllm_guided_decoding_regex else None
            )
