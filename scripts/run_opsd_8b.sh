#!/usr/bin/env bash
# Usage: RUN=<run_name> bash scripts/run_opsd_8b.sh
#
# vLLM server mode:
#   VLLM_MODE=server VLLM_SERVER_HOST=<node> VLLM_SERVER_PORT=8001 RUN=... bash scripts/run_opsd_8b.sh
...
RUN=${RUN:-qwen38b_opsd_$(date +%Y%m%d)}
